...
pub trait UpdateTrait<T: DocumentsTrait + serde::Serialize> {
    async fn update(es_cxt: &ElasticContext, folder_id: &str, doc_form: &T) -> WebResult<Successful>; 
}
...
impl UpdateTrait<DocumentPreview> for DocumentPreview {
    async fn update(
        ...
        let old_doc = es_cxt.get_document(folder_id, doc_form.get_doc_id()).await?;
        ...
        let ocr = old_doc
            .get_ocr_metadata()
            .map(|ocr_meta| OcrMetadata::builder()
